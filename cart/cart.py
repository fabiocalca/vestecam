...
class Cart:
    ...
    def add(self, product, unit_price, quantity=1.0):
        ...
        if item:
            ...
            item.save()
            Product.objects.filter(id=product.id).update(quantidade=F('quantidade')-1)
        else:
    ...
    def remove(self, product):
        item = models.Item.objects.filter(cart=self.cart, product=product).first()
        if item:
            Product.objects.filter(id=product.id).update(quantidade=F('quantidade')+item.quantity)
            item.delete()
        else:
